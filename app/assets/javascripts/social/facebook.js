with (scope('Facebook','App')) {
  ...
  define('get_app_id', function() {
    if (ST.www_host.match(/www\.bountysource\.com/)) {
      return "280280945425178";
    } else if (ST.www_host.match(/www-qa\.bountysource\.com/)) {
      return "129570157210142";
    } else if (ST.www_host.match(/www\.bountysource\.dev/)) {
...
//    options['data-width']       = options['data-width']       || 50;
  ...
  define('follow_button', function(options) {
    ...
    options['data-href']        = options['data-href']        || 'http://www.facebook.com/ST';
    options['data-layout']      = options['data-layout']      || 'button_count';
    options['data-show-faces']  = options['data-show-faces']  || false
    return div(options);
  });
  ...
  /*
  * Link Facebook to ST account via oauth redirect.
  * */
  define('auth_url', function(options) {
    options = options || {};
    return ST.api_host+'auth/facebook?scope='+(options.scope||'email') +
