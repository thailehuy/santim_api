with (scope('Signin','App')) {
  ...
  route('#signin/email', function() {
    ...
    render(
      ...
      p("Please enter your email address and a password. If you don't have a Santim account yet, we'll create one for you."),
  ...
  define('save_user_data_and_redirect', function(response) {
    ...
    ST.set_access_token(response.data);
  ...
  define('process_signin_form', function(refs, form_data) {
    ...
    ST.login(form_data, function(response) {
      if (response.meta.success) {
        ...
        ST.create_account(form_data, function(response) {
          if (response.meta.success) {
            save_user_data_and_redirect(response);
          } else {
  ...
  define('send_password_reset_email', function(refs, email) {
    ST.request_password_reset({ email: email }, function(response) {
      if (response.meta.success) {
  ...
  define('reset_password', function(error_div, form_data) {
    ...
    ST.reset_password(form_data, function(response) {
      if (response.meta.success) {
